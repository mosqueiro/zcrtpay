...
import { ZCRT,ethProvider } from '../contracts'
...
async function main() {
  ...
  for(let wallet of wallets) {
    ...
    let balanceBN = (await ZCRT.balanceOf(wallet.address))
    let balance = fromBN(balanceBN)
    console.log(`${wallet.address} ZCRT balance: ${balance} `)
    if (balance == 0) {
      ...
      if (options.collect) {
        ...
        console.log(`collecting ${balance} ZCRT from ${wallet.address}`)
        const ZCRTWithSigner = ZCRT.connect(new ethers.Wallet(wallet.privateKey, ethProvider))
        tx = await ZCRTWithSigner.transfer(options.collect, balanceBN)
        try {
          await tx.wait()
          console.log('done.')
          balanceBN = (await ZCRT.balanceOf(wallet.address))
