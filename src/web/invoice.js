...
import { classNames, displayZCRT } from './helpers'
  ...
  actions: {
    ...
    setInvoice: (invoice) => (state, actions) => {
      if (!state.invoice || state.invoice._id != invoice._id)
        QRCode.toDataURL(`ethereum:${invoice.deposit.address}/transfer?address=${config.contracts.ZCRT.address}&uint256=${invoice.totalAmount}`).then( result => actions.setQRCode(result))
      return {invoice}
    },
  ...
  view: ({match, state, actions}) => {
    ...
    if (invoice) {
      ...
      content = <div class="invoice">
        ...
        <tbody>
        { invoice.items.map( i => <tr><td>{ i.description }</td><td>{ displayZCRT(i.amount) } </td></tr>) }
        <tr><td><strong>Total:</strong></td><td>{ displayZCRT(invoice.totalAmount) }</td></tr>
        <tr><td><strong>Paid:</strong></td><td>{ displayZCRT(invoice.paidAmount) }</td></tr>
        </tbody>
        </table>
        { amountDue >= 0.01 ? <div>Transfer <strong>{ displayZCRT(amountDue) }</strong> to</div> : 'Deposit address:'} <div><strong>{ invoice.deposit.address }</strong></div>
        <div class="columns">
